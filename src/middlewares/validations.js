...
export const validateUser = async (req, res, next) => {
    ...
    if(validator.validateName(name).length > 0) data = [...data, {status: 400, field : 'name', errors : validator.validateName(name)}];

    if(validator.validateLastName(lastname).length > 0) data = [ ...data, {status: 400, field : 'lastname', errors : validator.validateLastName(lastname)}];
    ...
    if(errorsRoles.length > 0) data = [ ...data, {status: 400, field : 'roles', errors: errorsRoles}];
